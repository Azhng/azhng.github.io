...
- [Benchmark: the moment of truth](#benchmark)
...
1. JAX **traces** the Python function — runs it once with abstract values to record which ops happen
2. The trace becomes **HLO** (High-Level Operations) — a graph of ~100 primitives like `dot`, `reduce`, `broadcast`
3. XLA **optimizes** — most importantly, it **fuses** sequences of elementwise ops into single kernels so intermediates never hit HBM
...
Instead of two nested `fori_loop`s, `vmap` replaces the outer Q loop and `fori_loop` stays only for the inner K accumulation (which genuinely is sequential). Same algorithm, same tiles, same math — just giving the compiler one piece of information it didn't have before.
...
For anything beyond what XLA auto-selects, there's [Splash Attention](https://github.com/jax-ml/jax/tree/main/jax/experimental/pallas/ops/tpu/splash_attention) — Google's TPU-optimized flash attention written in Pallas. It uses DMA pipelining, MXU-matched tile sizes, and 2D grid scheduling — everything my `fori_loop` couldn't express.
...
|---|---|
| `jax.nn.dot_product_attention` | Default. XLA picks the best strategy. |
| Splash Attention (Pallas) | Long sequences at scale, kernel-level tuning beyond XLA. |
| Pure JAX `fori_loop` (what I wrote) | Understanding the algorithm. Not for production. |
